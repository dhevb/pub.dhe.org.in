...
const ReadArticle: React.FC = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const response = await axios.get('https://vie-rase-backend.onrender.com/api/vie_getallarticles');
        // const response = await axios.get('http://localhost:8080/api/vie_getallarticles');
      ...
      <ul className="space-y-4 mb-8">
        {articles.map((article) => (
          <li key={article.id} className="flex justify-between items-center p-4 bg-gray-100 rounded-lg shadow-sm hover:bg-gray-200 transition duration-200 ease-in-out">
            ...
            <a 
              href={`/vie.rase/ArticleDetail/${article.id}`}
              target="_blank" 
